...
import { Bell, Check, FileText, FileSignature, DollarSign, MessageSquare, MessagesSquare, Globe, Star, CreditCard, AlertTriangle, RotateCcw, ShieldAlert, ShieldCheck, ChevronDown, Clock, X, Archive } from 'lucide-react'
      ...
      <div className="flex items-center gap-1 shrink-0" onClick={e => e.stopPropagation()}>
        ...
        {supportsManage && (
          <button onClick={() => snooze(g.ids)} title="Remind me tomorrow" aria-label="Snooze"
            className="h-7 w-7 rounded-lg text-ink-faint hover:text-ink hover:bg-surface-raised flex items-center justify-center tap-target"><Clock className="w-3.5 h-3.5" /></button>
        )}
        <button onClick={() => dismiss(g.ids)} title={supportsManage ? 'Dismiss' : 'Mark read'} aria-label="Dismiss"
          className="h-7 w-7 rounded-lg text-ink-faint hover:text-ink hover:bg-surface-raised flex items-center justify-center tap-target">
                          ...
                          {g.items.map(n => {
                            ...
                            <div key={n.id} className="pl-16 pr-3 py-2.5 flex items-start gap-2">
                              ...
                              <div className="flex items-center gap-1 shrink-0">
                                ...
                                {supportsManage && <button onClick={() => snooze([n.id])} title="Remind me tomorrow" aria-label="Snooze" className="h-6 w-6 rounded-lg text-ink-faint hover:text-ink hover:bg-surface-raised flex items-center justify-center tap-target"><Clock className="w-3 h-3" /></button>}
                                <button onClick={() => dismiss([n.id])} title={supportsManage ? 'Dismiss' : 'Mark read'} aria-label="Dismiss" className="h-6 w-6 rounded-lg text-ink-faint hover:text-ink hover:bg-surface-raised flex items-center justify-center tap-target"><X className="w-3 h-3" /></button>
                              </div>
                            </div>
                            )})}
                        </div>
