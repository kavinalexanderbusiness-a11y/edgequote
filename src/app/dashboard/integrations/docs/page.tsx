...
const RESOURCES: { path: string; method: string; scope: string; note: string }[] = [
  ...
  { path: '/api/v1/customers', method: 'POST', scope: 'write', note: 'Find-or-create a customer (dedup by phone, then email); an address also finds-or-creates the property.' },
  { path: '/api/v1/customers/:id', method: 'GET', scope: 'read', note: 'One customer.' },
  { path: '/api/v1/quotes', method: 'GET', scope: 'read', note: 'List quotes. Extra filters: status, customer_id, property_id.' },
  { path: '/api/v1/quotes/:id', method: 'GET', scope: 'read', note: 'One quote.' },
  { path: '/api/v1/jobs', method: 'GET', scope: 'read', note: 'List jobs. Extra filters: status, customer_id, property_id, scheduled_date.' },
  { path: '/api/v1/jobs/:id', method: 'GET', scope: 'read', note: 'One job.' },
  { path: '/api/v1/invoices', method: 'GET', scope: 'read', note: 'List invoices. Extra filters: status, customer_id, property_id.' },
...
export function verifyEdgeHQ(secret, header, rawBody) {
...
export default function IntegrationDocsPage() {
  ...
  useEffect(() => {
    setBase(appOrigin(window.location.origin))
  }, [])
