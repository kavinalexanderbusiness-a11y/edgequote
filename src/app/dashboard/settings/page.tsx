          ...
          {localTiers.map((t, i) => (
            <div key={t.id || i} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-3 items-center">
              <input type="number" value={t.min_km ?? ''} onChange={e => updateTier(i, 'min_km', e.target.value)}
                className="bg-bg-tertiary border border-border-strong rounded-lg px-3 py-2 text-sm text-ink outline-none transition-all focus:border-accent focus:ring-2 focus:ring-accent/20" />
