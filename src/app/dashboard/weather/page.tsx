...
function Tile({ label, value, icon: Icon, accent }: { label: string; value: string; icon: typeof CloudRain; accent?: boolean }) {
  return (
    <Card className={cn('p-3.5', accent && 'border-accent/30 bg-accent/[0.05]')}>
      <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-ink-faint flex items-center gap-1.5"><Icon className="w-3 h-3" /> {label}</p>
      <p className={cn('text-xl font-black mt-1 tabular-nums', accent ? 'text-accent' : 'text-ink')}>{value}</p>
    </Card>
  )
}
