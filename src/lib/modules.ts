...
import {
  ...
  Calculator, HardHat, Target, Inbox, History, TrendingUp, BrainCircuit,
} from 'lucide-react'
...
export const FEATURE_MODULES: FeatureModule[] = [
  ...
  // The Inbox lists what needs the owner; the Operator REASONS about it — same
  // records, same canonical money/status engines, plus an answer to "why" and a
  // prepared next step. Core alongside Dashboard/Inbox because an attention
  // surface is not an optional feature. Its permission manifest is read-only BY
  // CONSTRUCTION in Phase 1: no :write or messages:send appears here until the
  // approval seam ships, so the listing never promises more than the code can do.
  { key: 'operator',   label: 'Operator',   href: '/dashboard/operator',   icon: BrainCircuit, core: true,
    category: 'operations', version: 1, updatedAt: '2026-08-31', featured: true,
    description: 'Ask what needs attention and get evidence-backed, read-only recommendations.',
    permissions: ['customers:read', 'messages:read', 'quotes:read', 'jobs:read', 'invoices:read',
      'payments:read', 'expenses:read', 'crews:read', 'automations:read'],
    keywords: 'assistant ai operator brief priorities attention ask edgequote recommendations read only' },
