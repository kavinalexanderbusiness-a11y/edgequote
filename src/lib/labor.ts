...
// Service identity (SERVICE_DEFS/serviceKey/serviceLabel) moved to the
// dependency-free leaf lib/serviceKey — labor.ts imports the pricing engine, and
// lib/dedup (in the always-mounted upload widget) needs only serviceKey. Imported
// for this file's own use AND re-exported so all existing importers are unchanged.
import { serviceKey, serviceLabel } from '@/lib/serviceKey'
export { serviceKey, serviceLabel } from '@/lib/serviceKey'
