...
export function BulkActionBar({ count, actions, onClear, busyKey }: {
  ...
  return (
    <div className="sticky top-2 z-20 flex items-center gap-1.5 flex-wrap bg-bg-secondary border border-accent/40 rounded-xl px-3 sm:px-4 py-2.5 shadow-lg origin-top animate-pop">
        ...
        return (
          <button key={a.key} type="button" onClick={a.onClick} disabled={a.disabled || !!busyKey} aria-busy={busy || undefined}
            className={cn(
              ...
              a.tone === 'danger' ? 'border-red-500/30 text-red-300 hover:bg-red-500/10 [[data-theme=light]_&]:text-red-800'
      ...
      <button type="button" onClick={onClear} className="ml-auto inline-flex items-center gap-1 text-xs font-medium text-ink-faint hover:text-ink [[data-theme=light]_&]:text-ink-muted [[data-theme=light]_&]:hover:text-ink rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40">
