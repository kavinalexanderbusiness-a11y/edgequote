  ...
  return (
    <div className={cn('grid grid-cols-2 lg:grid-cols-4 gap-3', className)}>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="rounded-card border border-border bg-bg-secondary p-3.5">
...
export function SkeletonRows({ count = 5, className }: { count?: number; className?: string }) {
  return (
    <div className={cn('rounded-card border border-border bg-bg-secondary divide-y divide-border overflow-hidden', className)}>
      {Array.from({ length: count }).map((_, i) => (
