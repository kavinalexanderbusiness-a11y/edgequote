...
export function PriceIntelligence({
  ...
  return (
    <div className={cn('rounded-xl border p-3 space-y-2.5', rec.enoughData ? 'border-accent/25 bg-accent/[0.05]' : 'border-border bg-bg-tertiary')}>
      ...
      <div className="flex items-end justify-between gap-3">
        ...
        {!applied
          ? <button type="button" onClick={() => onApply(rec.price)} className="shrink-0 text-xs font-bold rounded-lg px-3 py-1.5 bg-accent text-white hover:opacity-90">Use ${rec.price}</button>
          : <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-400 shrink-0"><Check className="w-3.5 h-3.5" /> Applied</span>}
      </div>
