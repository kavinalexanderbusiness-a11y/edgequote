...
// Global command palette — universal search (customers, quotes, invoices, jobs,
// messages) + quick actions. Opens on Cmd/Ctrl+K or the `eq:command-open` event.
// Server-side ilike search scoped by user_id keeps it instant on large datasets.
export function CommandPalette() {
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(async () => {
      ...
      // Strip characters that would break the PostgREST or()/ilike grammar.
  ...
  const overlay = (
    <div className="fixed inset-0 z-[200] flex items-start justify-center px-4 pt-[14vh] sm:pt-[12vh]"
      onMouseDown={close}>
      ...
      <div role="dialog" aria-label="Command palette" onMouseDown={e => e.stopPropagation()}
        ...
        <div className="hidden sm:flex items-center gap-3 px-4 py-2 border-t border-border text-[10px] text-ink-faint shrink-0">
          ...
          <span className="ml-auto">Universal search</span>
        </div>
      </div>
    </div>
  )
