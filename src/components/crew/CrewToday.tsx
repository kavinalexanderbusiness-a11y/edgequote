  ...
  async function act(stop: ActiveCrewStop, kind: 'start' | 'complete' | 'stop') {
    ...
    try {
      ...
      if (res.state === 'failed') {
        ...
        // items land on the card, beside the checklist they belong to. It
        // survives the resilience layer because runVisitIntent carries the
        // server's list through: a deterministic 422 reconciles to `unapplied`
        // (nothing changed), so this is a real refusal with a real remedy, not
