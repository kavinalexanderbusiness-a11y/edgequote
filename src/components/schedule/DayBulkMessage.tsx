...
import {
  ...
  X, Send, Check, AlertTriangle, MessageSquare, Mail, Users,
} from 'lucide-react'
...
export function DayBulkMessage({ date, jobs, onClose }: { date: string; jobs: Job[]; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-start justify-center p-4 sm:p-6 bg-black/50 backdrop-blur-sm overflow-y-auto" onClick={onClose}>
      <div role="dialog" aria-modal="true" aria-label="Message today’s customers" className="w-full max-w-lg bg-bg-secondary rounded-card border border-border-strong shadow-xl mt-6 mb-6" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-border">
          ...
          <button type="button" onClick={onClose} className="h-7 w-7 rounded-lg flex items-center justify-center text-ink-faint hover:text-ink transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40" aria-label="Close"><X className="w-4 h-4" /></button>
        </div>
