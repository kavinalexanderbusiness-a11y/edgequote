      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map(c => {
            ...
            return (
              <Card key={c.jobId} className="overflow-hidden flex flex-col">
                <div className="relative h-40 bg-bg-tertiary">
                  ...
                  <span className={cn('absolute top-2 right-2 text-[10px] font-bold rounded-full px-1.5 py-0.5 border', toneSoft[scoreTone(c.score)])}>
                    {c.score}
                  </span>
                </div>
                <div className="p-3 flex-1 flex flex-col gap-1.5">
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap mt-0.5">
                    {c.hasBefore && c.hasAfter && <span className="inline-flex items-center gap-1 text-[10px] text-accent"><ArrowLeftRight className="w-3 h-3" /> B&amp;A</span>}
                  ...
                  <Link
                    href={`/dashboard/grow/studio?job=${c.jobId}`}
                    className="mt-auto inline-flex items-center justify-center gap-1.5 rounded-xl bg-accent/10 border border-accent/20 text-accent text-xs font-semibold py-2 hover:bg-accent/15 transition-colors"
                  >
