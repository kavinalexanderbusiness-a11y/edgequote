...
console.log('\n── §1 one owner, leased writes, named first, cleared at sign-out ──')
{
  ...
  const DEFERRED: string[] = [] // the two customer pages were threaded once their owner handed them over
  ...
  check('discovery sees every writer, generic calls included (14 files, all threaded)', writers.length === 14, `found ${writers.length}: ${writers.join(', ')}`)
