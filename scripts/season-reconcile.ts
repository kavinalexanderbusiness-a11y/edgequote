...
async function main() {
  ...
  // ── The OWNER REVIEW dossier ───────────────────────────────────────────────
  // ⭐ Everything Kavin needs to decide, per series, in one place. ⛔ It changes
  // nothing and recommends no default: the whole reason these rows are here is
  // that no rule can classify them, so this offers CHOICES rather than a guess.
  const review = by('OWNER REVIEW')
  if (review.length) {
    console.log(`\n════ 1b · OWNER REVIEW — the ${review.length} series a rule cannot classify ════`)
    for (const r of review) {
      const js = byRec.get(r.id) ?? []
      const done = js.filter((j: any) => j.status === 'completed').map((j: any) => j.scheduled_date).sort()
      const future = js.filter((j: any) => j.scheduled_date > today && j.status !== 'cancelled')
        .map((j: any) => j.scheduled_date).sort()
      console.log(`\n  ── ${r.who} ──`)
      console.log(`     recurrence id        ${r.id}`)
      console.log(`     customer             ${r.who}`)
      console.log(`     service              "${r.service}"`)
      console.log(`     cadence              ${r.cadence}`)
      console.log(`     start_date           ${r.start}`)
      console.log(`     current end_date     ${r.end ?? 'NONE — nothing bounds this series'}`)
      console.log(`     completed history    ${done.length ? `${done[0]} → ${done[done.length - 1]} (${done.length} visit(s))` : 'NONE — no completed work'}`)
      console.log(`     future generated     ${future.length ? `${future[0]} → ${future[future.length - 1]} (${future.length} visit(s))` : 'none'}`)
      console.log(`     seasons available    ${Object.keys(seasons).sort().map(k => `${k} (${seasonLabel(seasons[k])})`).join(' · ')}`)
      console.log(`     legacy inference     ${r.suggested ?? 'NO MATCH — the keyword lists return nothing for this name'}`)
      // Why a rule cannot decide, stated concretely rather than as a category.
      const reasons: string[] = []
      if (!r.suggested) reasons.push('the service name matches no keyword in any configured season, so there is nothing to suggest')
      if (!done.length) reasons.push('there is no completed history, so no evidence of which months it actually runs')
      else {
        const covered = Object.keys(seasons).filter(k => done.some(d => isWithinSeason(d, seasons[k])))
        reasons.push(covered.length === 0
          ? 'its completed work falls outside every configured season'
          : `its completed work falls only inside ${covered.join(' + ')}, which is consistent with that season AND with year-round`)
      }
      reasons.push('⛔ the future visits cannot be used as evidence — they were GENERATED by the very defect being repaired')
      console.log(`     cannot auto-classify because:`)
      for (const why of reasons) console.log(`       • ${why}`)
      console.log(`     recommended choices (Kavin picks one; nothing is applied here):`)
      for (const k of Object.keys(seasons).sort()) {
        const out = future.filter(d => !isWithinSeason(d, seasons[k])).length
        console.log(`       assign "${k}"   → end_date becomes ${effectiveSeriesEnd(r.start, r.end, seasons[k])}`
          + `, ${out} future visit(s) would then be out of season`)
      }
      console.log(`       Year-round     → season_key = '${SEASON_NONE}'; nothing is out of season, but the series still needs an end date or it regenerates forever`)
      console.log(`       Keep blocked   → leave season_key NULL; the series stays undeclared and`)
      console.log(`                        SEASON_DECLARATIONS_COMPLETE cannot be flipped`)
    }
  }
