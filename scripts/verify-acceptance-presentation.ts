...
{
  const p = acceptedPresentation(LIVE.status, LIVE.evidence > 0)
...
{
  const evidenced = acceptedPresentation('accepted', true)
  check('evidence present → "evidenced"', evidenced === 'evidenced')
  ...
  check('hasEvidence=false refuses the claim', acceptedPresentation('accepted', false) === 'unevidenced')
  check('hasEvidence=undefined (unknowable) ALSO refuses — fails closed',
...
console.log('\n■ 4. The portal model is wired to the rules, not to status')
{
  ...
  check('the model asks acceptedPresentation, never status alone',
    /acceptedPresentation\(qq\.status, qq\.has_acceptance_evidence\)/.test(model))
  ...
  check('the consent sentence is reachable only through the evidenced branch',
    /priceMovedSinceAccepted\s*\r?\n?\s*\?\s*'This is the price you accepted/.test(model)
    && /const acceptedFigure = facing\.isAcceptedAmount \? facing\.amount : null/.test(model))
  check('the unevidenced state is NAMED to the customer',
    /presentation === 'unevidenced' \? unevidencedAcceptanceNote\(\)/.test(model))
  ...
  check('the payload flag is optional (absent = unproven, not false-negative)',
    /has_acceptance_evidence\?: boolean/.test(model))
  ...
  check('…and it reports only WHETHER consent exists, never its detail',
    /exists \(select 1 from public\.quote_acceptances/.test(sql)
    && !/accepted_amount|accepted_at|actor/.test(sql))
}
