...
// ⚠️ A migration has TWO lives. While it is in flight it is its own file in
// supabase/migrations/. Once production has run it and the baseline is
// regenerated, it is ABSORBED: the file moves to supabase/archive/ledger/ (which
// is never applied) and the column, its CHECK and its nullability live in the
// baseline instead. Both states are correct, so the rule this guard defends is
// not "a file exists" — it is "the DB, not just TypeScript, owns the allowed
// set, and it got there through the apply path". Asserting the file made the
// guard fail the moment the schema converged, which is the guard being wrong
...
const baselineName = migFiles.find(f => /_baseline\.sql$/.test(f))!
const standalone = migFiles.find(f => /_customer_preferred_channel\.sql$/.test(f))
const applyPath = migFiles.map(f => readFileSync(join(ROOT, 'supabase', 'migrations', f), 'utf8')).join('\n')
check('the preferred_channel column reaches production through the apply path',
  /preferred_channel/.test(applyPath), true)
check(`…carried by ${standalone ? 'its own migration' : 'the generated baseline (absorbed)'}`,
  standalone ? /preferred_channel/.test(readFileSync(join(ROOT, 'supabase', 'migrations', standalone), 'utf8'))
             : /preferred_channel/.test(readFileSync(join(ROOT, 'supabase', 'migrations', baselineName), 'utf8')), true)
// Only meaningful while the migration is still a separate file: verify:rebuild
// applies the baseline and everything after it in FILENAME order, so a name that
// sorted earlier would add a column to a table that does not exist yet.
if (standalone) {
  check('…and sorts after the generated baseline', standalone > baselineName, true)
}
// ⚠️ ONE state, TWO spellings. A hand-written migration says
//   check (preferred_channel is null or preferred_channel in ('sms','email','phone'))
// and the generated baseline says
//   CHECK (((preferred_channel IS NULL) OR (preferred_channel = ANY (ARRAY['sms'::text,…]))))
// A guard pinned to either literal goes green today and silently stops checking
// after the next resync, so assert the STATE: one CHECK naming the column, that
// admits NULL and exactly these three values — no more.
const checkLine = applyPath.split('\n').find(l => /preferred_channel/i.test(l) && /\bcheck\s*\(/i.test(l)) || ''
const admits = (v: string) => new RegExp(`'${v}'`).test(checkLine)
...
check('the column is nullable (no preference is a real state)', /not null/i.test(colLine), false)
