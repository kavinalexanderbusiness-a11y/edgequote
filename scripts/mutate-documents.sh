...
SCHEMA="supabase/pending/2026-08-15-documents-signatures-v1.sql"
...
# signal we want is whether THIS NAMED CHECK appears with a ✗.
run_static() {
  npx tsx scripts/verify-documents.ts 2>/dev/null
}
...
mutate() {
  ...
  if ! diff -q "$file" "$BACKUP_DIR/$([ "$file" = "$SCHEMA" ] && echo schema.sql || echo pad.tsx)" >/dev/null; then
    local out; out="$(run_static)"
    if echo "$out" | grep -F "✗" | grep -qF "$expect"; then
      echo "  ✓ MUTATION CAUGHT — $name"
      pass=$((pass+1))
    else
      echo "  ✗ MUTATION SURVIVED — $name"
      echo "      expected a ✗ on: $expect"
      fail=$((fail+1))
    fi
  else
    echo "  ✗ MUTATION DID NOT APPLY — $name (the pattern no longer matches; fix this script)"
    fail=$((fail+1))
  fi
  restore
}
...
mutate "portal_get_documents projects the storage path" \
  ...
  "s/'file_name', v\.file_name,/'file_name', v.file_name, 'storage_path', v.storage_path,/"
...
echo "──────────────────────────────────────────────────────────────────────"
if [ "$fail" -eq 0 ]; then
  echo "✅ mutation test — $pass/$((pass+fail)) mutations caught. Every rule can go red."
else
