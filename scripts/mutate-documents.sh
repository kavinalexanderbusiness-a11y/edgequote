#!/usr/bin/env bash
# ââ Mutation test for verify:documents âââââââââââââââââââââââââââââââââââââââ
#
...
# takes. The behavioural half proves itself by construction â every one of its
...
#
# â ï¸ It edits real files and restores them from a backup in a trap, so an
...
# â­ HIDE PGLITE FOR THE DURATION. The guard treats PGlite as an OPTIONAL
# dependency and skips its behavioural half cleanly when the package is absent â
...
run_static() {
  # ⚠️ 2>&1, NOT 2>/dev/null: check() writes failures with console.error, so
  # discarding stderr throws away every ✗ and reports "0 caught" for a guard
  # that was working perfectly. A mutation harness that cannot see failures is
  # itself the thing most worth testing.
...
mutate() {
  ...
  # ⚠️ Match the failure marker by its UTF-8 BYTES (E2 9C 97), never by the glyph.
  # Git-Bash's locale mangles ✗ on the way through, so a glyph comparison matched
...
echo ""
echo "ââ mutation test Â· verify:documents ââââââââââââââââââââââââââââââââââââ"
# 18 Â· assignment regresses to the crew-only predicate (S65's model ignored)
...
# 19 Â· the null guard moves back to the crew, refusing crewless technicians
...
# 1 Â· privacy: the bucket becomes public
...
# 2 Â· the storage policy stops being owner-scoped
...
# 3 Â· entity link: exactly-one becomes at-most-one
...
# 4 Â· visibility stops defaulting to the safe value
...
# 5 Â· worker visibility no longer requires a visit
...
# 6 Â· tenancy: anon is handed the table
...
# 8 Â· signatures become editable
...
# 9 Â· the signer becomes client-supplied
...
# 12 Â· the crew door widens to the customer's copy
...
# 13 Â· audit starts carrying the drawn mark
...
# 14 Â· the audit apply-order precondition is deleted
...
# 15 Â· documents grows its own audit engine
...
# 16 Â· mobile: the pad stops claiming the gesture
...
# 17 Â· mobile: the DPR backing store is dropped
...
if [ "$fail" -eq 0 ]; then
  ...
  echo "â mutation test â $fail SURVIVED, $pass caught."
