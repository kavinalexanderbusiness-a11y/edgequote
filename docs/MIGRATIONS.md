...
Before 2026-08-13 there was no migration system — there was a convention, and the
convention decayed:

- `supabase/schema.sql` was a snapshot from 2026-06-25 and drifted seven weeks behind.
- `supabase/RUN-*.sql` grew to 99 files applied "in filename order". They were never
  a complete history: **30 migrations reached production leaving no repo file at all.**
- `get_portal_data` had been defined in nine RUN files *and* eleven times inside
  ...
  just started showing less. That happened twice.
...
cannot be applied, and two guards that fail loudly.
...
supabase/
  migrations/          ← THE APPLY PATH. Run these, in filename order, and nothing else.
    20260814040000_baseline.sql
  archive/             ← history. NEVER applied. Kept so "why is this column here?" is answerable.
    ledger/            ← the 118 migrations production actually ran, recovered from its ledger
...
Its version (`20260814040000`) sorts **after** every migration folded into it, so a
