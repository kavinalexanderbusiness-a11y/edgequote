...
--
--   version : 20260715233509
--   name    : accounting_foundation_expenses_vendors_categories
--
...
-- ══════════════════════════════════════════════════════════════════════════════
-- Accounting foundation — money OUT. Expenses, vendors, categories.
--
...
-- engine AutoPay, Stripe, reconciliation, dunning and receipts already depend on.
-- payments stays untouched. Reporting reads BOTH and lives in ONE engine
-- (lib/accounting) — never a second accounting model.
--
...
-- Cash flow uses `amount` (it must reconcile to the bank). P&L uses net (tax
-- isn't an expense — it's recoverable). Storing gross + tax makes both derivable
-- from ONE row, so the two reports can never disagree. Storing net instead would
-- make cash flow unreconcilable without re-deriving tax.
--
