...
--
--   version : 20260715075726
--   name    : portal_get_portal_data_union_service_plans_and_expiry
--
...
-- get_portal_data — UNION of the two live lineages, restoring a regression.
--
-- The production function combined TWO branches that never met in git:
--   * RUN-2026-07-14-portal-service-plans.sql (guardian/dedup branch, applied to
--     prod, NOT on main) — service_seasons + job/recurrence fields that
--     lib/recurrence.buildServicePlans needs.
--   * RUN-2026-07-15-portal-quote-expiry.sql (main) — qt.valid_until.
-- Applying the main version alone silently dropped the service-plan fields.
-- This is the superset of both. Anything replacing it must carry BOTH lineages.
