...
--
--   version : 20260715232348
--   name    : portal_services_fix_sort_order
--
...
-- HOTFIX for the migration immediately prior: `json_agg(s order by s.sort_order, s.name)`
-- referenced a column the inline subquery didn't select, so get_portal_data raised
-- 42703 for EVERY customer — the whole portal, not just the new services key.
-- Caught by executing the function against a real token rather than inspecting its
-- text. Same body, with sort_order selected so the ORDER BY can see it.
