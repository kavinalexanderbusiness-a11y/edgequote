...
--   name    : portal_tenant_scoped_lookups
--
-- Applied to production 2026-08-16 via the management API (Session 75) and
-- recorded in supabase_migrations.schema_migrations. Folded into the baseline
-- by npm run schema:baseline.
--
-- ═══════════════════════════════════════════════════════════════════════════

-- ── The portal proves BOTH the customer and the tenant ───────────────────────
--
...
-- ever became inconsistent by some other route — a service_role write, a
-- restore, a future migration — still cannot read or move another tenant's data.
--
-- ⚠️ EVERY BODY BELOW WAS READ FROM PRODUCTION with pg_get_functiondef and
...
-- ── get_portal_data — 11 customer-keyed projections ─────────────────────────
...
-- ── portal_accept_quote — 1 customer-keyed clause(s) scoped ──
...
-- ── portal_add_contact — 3 customer-keyed clause(s) scoped ──
...
-- ── portal_begin_setup — 1 customer-keyed clause(s) scoped ──
...
-- ── portal_decline_review — 1 customer-keyed clause(s) scoped ──
...
-- ── portal_invoice_for_payment — 1 customer-keyed clause(s) scoped ──
...
-- ── portal_mark_reviewed — 1 customer-keyed clause(s) scoped ──
...
-- ── portal_remove_card — 3 customer-keyed clause(s) scoped ──
...
-- ── portal_set_autopay — 2 customer-keyed clause(s) scoped ──
...
-- ── portal_set_consent — 1 customer-keyed clause(s) scoped ──
...
-- ── portal_set_scheduling_preference — 1 customer-keyed clause(s) scoped ──
...
-- ── portal_submit_request — 1 customer-keyed clause(s) scoped ──
