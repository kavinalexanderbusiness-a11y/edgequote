...
--
--   version : 20260816043000
--   name    : crews_team_assignments_v1
--
-- ⚠️ RECOVERED FROM THE PRODUCTION LEDGER by Session 75, not authored here.
-- Session 65 applied this to production; the repo had no file for it, which
-- npm run verify:migrations reports as "applied with NO repo file". The SQL
-- below is exactly the text production executed, read back from
-- supabase_migrations.schema_migrations.statements. Archiving it makes the
-- repository describe reality; it does not land session 65's application code.
--
-- ═══════════════════════════════════════════════════════════════════════════
