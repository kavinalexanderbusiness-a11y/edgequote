...
--
--   version : 20260814074915
--   name    : quote_addons_v1_choice_core
--
...
-- ══════════════════════════════════════════════════════════════════════════════
-- QUOTE ADD-ONS V1 — ONE WRITER, TWO DOORS
-- ══════════════════════════════════════════════════════════════════════════════
-- The portal (customer token) and the owner's screen (customer rang up) both need
-- to say "this is what was bought". Two functions each knowing the money rule =
-- two answers to that question. So both delegate to ONE core that authorises
-- nothing and is granted to no role:
--
--   portal_accept_quote(token, quote, option?, addons?)  → token resolves a customer
--   owner_select_quote_option(quote, option?, addons?)   → auth.uid() owns the quote
--                           ↓  both delegate to  ↓
--            quote_apply_choice(quote, option, addon_ids, via)
--
-- ⭐ Renamed from quote_apply_option_choice: it now applies the WHOLE decision —
--    which alternative, and which extras. It is granted to no role and called by
--    nothing but the two doors, so the rename cannot break a caller.
-- ⛔ The DOOR names are deliberately NOT renamed. The deployed frontend calls
--    them by name; a rename would break production the moment this migration
--    lands, hours before the code that needs it. Both gained defaulted
--    parameters, so an existing 2- or 3-argument call still resolves.

drop function if exists public.quote_apply_option_choice(uuid, uuid);
...
begin
  ...
  -- an extra the customer untick ed must stop being selected, or a pre-ticked
  -- suggestion would be billed because nobody said no loudly enough.
  -- The quote is still 'sent' at this instant, so the freeze trigger allows it;
  -- the status flip below is what closes the set for good.
...
-- ⚠️⚠️ Naming every role is the only thing that removes Supabase's
-- ALTER DEFAULT PRIVILEGES grants, which are handed out at CREATE time.
-- `revoke ... from public` alone leaves anon holding EXECUTE.
revoke all on function public.quote_apply_choice(uuid, uuid, uuid[], text)
  from public, anon, authenticated, service_role;

-- ── Door 1: the owner records the customer's decision ─────────────────────────
drop function if exists public.owner_select_quote_option(uuid, uuid);
create or replace function public.owner_select_quote_option(
  p_quote_id uuid, p_option_id uuid default null, p_addon_ids uuid[] default null
) returns boolean
language plpgsql security definer set search_path to 'public' as $fn$
begin
...
-- ── Door 2: the customer approves from their portal ───────────────────────────
drop function if exists public.portal_accept_quote(text, uuid, uuid);
create or replace function public.portal_accept_quote(
  p_token text, p_quote_id uuid, p_option_id uuid default null, p_addon_ids uuid[] default null
) returns boolean
language plpgsql security definer set search_path to 'public' as $fn$
...
revoke all on function public.owner_select_quote_option(uuid, uuid, uuid[]) from public, anon, service_role;
grant execute on function public.owner_select_quote_option(uuid, uuid, uuid[]) to authenticated;

revoke all on function public.portal_accept_quote(text, uuid, uuid, uuid[]) from public, service_role;
grant execute on function public.portal_accept_quote(text, uuid, uuid, uuid[]) to anon, authenticated;

-- Hardening, same feature, same class of hole: anon held table-level DML on
-- quote_options from Supabase's create-time default privileges. RLS already
-- refused it every time (auth.uid() is null there), but the grant had no reason
-- to exist and its absence is what the new sibling table relies on.
revoke all on table public.quote_options from anon;