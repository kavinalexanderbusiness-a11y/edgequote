...
-- APPLIED to production 2026-08-11 via MCP (verified before and after).
...
-- ════════════════════════════════════════════════════════════════════════════
--
-- WHY. `customers.acquisition_source` is the canonical record of where a customer
-- came from, and TWO of the strings that reach it come from a public URL:
-- `submit_booking` does
--     v_source := coalesce(nullif(trim(p_hear_about), ''), nullif(p_utm->>'source', ''), 'Online Booking')
-- and `submit_booking` is granted to `anon`. So `?utm_source=<anything>` — any
-- length, newlines, control characters — landed verbatim in the column every
-- acquisition report reads. That is a public field poisoning internal reporting.
--
-- ⭐ WHAT THIS DELIBERATELY DOES NOT DO: it does not enforce a vocabulary in SQL.
-- Whitelisting channel names here would mean a second copy of the mapping in
-- src/lib/attribution.ts, in another language, free to drift. The database's job
-- is to keep the value SANE (bounded, printable); deciding what it MEANS is the
-- app normalizer's job, and it folds anything it doesn't recognise into "Other"
-- with the raw text kept — so report cardinality is bounded no matter what is
-- sent. Raw `hear_about`/`utm`/`referral_code` continue to be stored verbatim on
-- the draft quote's `lead_meta`; nothing is lost, only bounded on its way into
-- the reporting column.
--
-- ⛔ supabase/schema.sql is NOT updated by this file, on purpose: it still holds
-- the PRE-BK-1 bodies of the intake doors plus a `drop function ... submit_booking`
-- (~line 1439), so running it would already revert BK-1's shared identity seam.
-- Adding this change there would only deepen that trap. Live DB + this file are
-- the truth (see the lead-intake-two-paths note; reconciliation is INF-2's).

-- ── 1. The sanitiser ────────────────────────────────────────────────────────
-- Mirrors sanitizeSourceInput() in src/lib/attribution.ts: strip control
-- characters, collapse whitespace, trim, bound to 60 characters, empty → null.
-- Pure text in / text out: no table access, so SECURITY INVOKER (the default) is
-- correct — it runs with the definer's rights only because the intake doors that
-- call it are themselves SECURITY DEFINER.
...
-- Least privilege, and the lesson from the tenant-boundary audit: revoking from
-- anon/authenticated is NOT enough on its own, because a function in `public` is
-- granted to PUBLIC by default. Revoke that first, then the roles.
...
-- ── 2. The public booking door ──────────────────────────────────────────────
-- Byte-identical to the live body EXCEPT the v_source line. Signature, grants
-- (CREATE OR REPLACE preserves them), search_path and every other statement are
-- unchanged.
...
begin
  ...
  -- ⭐ THE ONLY CHANGED LINE. Both candidates are public input; both are bounded
  -- and stripped before they can become an acquisition source. The raw values
  -- still go to v_meta below, untouched.
  v_source := coalesce(
...
-- ── 3. The shared identity seam ─────────────────────────────────────────────
-- Byte-identical to the live body EXCEPT the two acquisition_source lines.
-- BK-1's match order (phone last-10 → email → address) and the blank-only
-- backfill of phone/email are untouched.
--
-- The INSERT branch is sanitised because `submit_booking` is not the only public
-- caller: `/api/intake` is CORS-open and unauthenticated, and takes `source`
-- verbatim from the request body → submit_website_lead(p_source) → here. Both
-- doors now pass through the same bound. (The TS side does it too, in
-- lib/intake.submitLead — but the RPC is granted to `anon`, so the database is
-- the boundary that actually holds.)
--
-- FIRST-TOUCH, PRECISELY: the source is written when the customer row is created
-- and is NEVER overwritten by a later touch. What changes is the case where the
-- first touch recorded NOTHING: a customer added by hand with the source left
-- blank, who later arrives through a real door, kept `null` forever — the door
-- knew where they came from and threw it away. `coalesce` fills only the blank,
-- so a known source can still never be replaced. The contract is therefore
-- "first KNOWN touch wins", which is stated plainly wherever it is reported.
