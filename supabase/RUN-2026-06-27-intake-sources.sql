...
-- Shared lead intake — generalize submit_website_lead with a SOURCE.
-- Idempotent + backward compatible. Mirrors supabase/schema.sql.
-- ════════════════════════════════════════════════════════════
--
-- One pipeline already de-dupes the customer, creates the property, writes the
-- lead, threads it into Messages and notifies the owner. The ONLY thing that
-- varied by channel was the hardcoded acquisition_source = 'Website'. This adds an
-- optional p_source so every external source (website contact / quote / booking
-- forms, Formspree, a generic webhook, future integrations) reuses the SAME
-- pipeline while recording where the lead came from. No new tables, no duplicate
-- logic. Existing 2-arg callers (the /api/website-lead route) keep working — the
-- new 3rd arg defaults to 'Website'.

-- MIGRATION 2026-06-27 — intake sources
-- Drop the prior 2-arg signature, then (re)create with the optional p_source.
