...
-- ── MIGRATION 2026-06-24h — Spotlight conversation search (supersedes 24g's RPC).
-- Adds PROPERTY (address/city) and SERVICE (job/quote service_type) matching, a
-- match_type so the UI can label why a result matched, and a snippet of the actual
-- matching message so search shows the relevant line (not just the last preview).
-- Still returns archived conversations and is token-scoped via auth.uid(). Idempotent.
create or replace function public.search_conversations(p_query text)
returns json language plpgsql security definer set search_path = public as $$
declare v_user uuid := auth.uid(); q text := '%' || trim(coalesce(p_query, '')) || '%'; result json;
begin
...
-- ── Lead intake: accept live website payload keys (backward-compatible aliases, 2026-07-14) ──
-- See supabase/RUN-2026-07-14-lead-key-aliases.sql. Not a schema migration.
create or replace function public.submit_website_lead(p_token text, p_payload jsonb, p_source text default 'Website')
returns json language plpgsql security definer set search_path = public as $$
declare
  v_user uuid; v_customer uuid; v_prop uuid; v_lead uuid; v_convo uuid;
...
-- See supabase/RUN-2026-07-14-delivery-tracking.sql. Additive + idempotent.
...
-- changes (production bug: INV-0025 discounted to exactly its paid amount stayed
-- 'partial' with a $0 balance, paid_at never stamped).
--
-- recompute_invoice_paid derives amount_paid/status/paid_at from the ledger, but
-- its only trigger sat on PAYMENTS — editing the invoice changes the other half
-- of the equation (the total) and nothing refired the engine. The status rule is
-- UNCHANGED: the body moves verbatim into a core function; the payments trigger
-- delegates to it, and a new trigger on invoices delegates to it when a
-- money-defining column actually changes. One engine, two ignitions.
--
-- AFTER + nested UPDATE (not BEFORE mutating NEW) so the nested UPDATE lists
-- `status` in SET — firing the existing AFTER UPDATE OF status triggers (owner
-- notification, quote sync, integration capture) exactly like a payment does.
-- No recursion: the nested UPDATE sets only amount_paid/paid_at/status, none of
-- which are in the new trigger's column list. See
-- RUN-2026-07-26-invoice-status-on-total-edit.sql. Applied 2026-07-26.
-- ════════════════════════════════════════════════════════════
...
-- ============================================================
-- MIGRATION 2026-08-11 - Deposit-gated scheduling V1 (mirror of
-- RUN-2026-08-11-deposit-gated-scheduling.sql - applied live 2026-08-11).
-- get_portal_data gained quote deposit/preference fields + payments.quote_id
-- in the same pass - see CANONICAL-get_portal_data.sql (THE definition).
-- ============================================================
